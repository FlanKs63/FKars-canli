...
def fetch(day: datetime, session=None) -> list[dict]:
    import requests
    session = session or requests
    body = {"fromDate": str(day.date() - timedelta(days=1)), "toDate": str(day.date()),
...
def check(watch: set[str], seen: list, now: datetime, send, session=None, first_run: bool = False) -> int:
    """Yeni ve önemli bildirimleri gönderir. seen: gönderilen/görülen disclosureIndex listesi (state).
    İlk çalıştırmada (seen boş) geçmiş bildirimleri göndermeden sadece 'görüldü' işaretler."""
    items = fetch(now, session)
    if not items:
        return 0
    known = set(map(str, seen))
    sent = 0
    ...
    for item in sorted(items, key=lambda d: int(d.get("disclosureIndex") or 0)):
        ...
        if not idx or idx in known:
            continue
        seen.append(idx)
        known.add(idx)
        codes = [c for c in tickers(item) if c in watch]
        if first_run or not codes or not is_important(item) or sent >= limit:
            continue
        send(message(item, codes))
        sent += 1
        print(f"  KAP {','.join(codes)}: {item.get('subject')}")
    del seen[:-2000]                                   # state şişmesin
    return sent
